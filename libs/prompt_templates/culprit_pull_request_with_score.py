...
- Code diff - the actual code changes in diff format (truncated for readability). Use this to understand the technical details of changes.
...
5. **Carefully examine the code diff** to understand what functions, variables, conditions, or logic were modified. Look for changes that could directly impact the issue reproduction flow.
6. Cross-reference the **files changed** with the issue context - PRs touching relevant files are more likely to be culprits.
7. Provide the PRs most likely responsible for the issue.
8. If multiple PRs have a good score, reason which one is more likely to be the culprit.
9. Rank the PRs based on how the flow of issue reproduction steps matches with test steps of each PR.
10. Use both the code diff summary and the actual code diff to understand what changed and if they are related to the issue reproduction steps, consider it more likely to be the culprit.
11. PRs that change unrelated areas or cannot affect the described flow should not be considered.
12. Keep the reason concise and one sentence long and based on code diff analysis, code diff summary, and test steps.
13. Return top {culprits_to_find} most likely culprit PRs only.
